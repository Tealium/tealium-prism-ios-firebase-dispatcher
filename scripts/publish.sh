...
if [ "$PUSH" = true ]
then
    bundle exec pod trunk push TealiumPrismFirebase.podspec
else
    echo "Do you wish to publish to CocoaPods?"
    select yn in "Yes" "No"; do
        case $yn in
            Yes ) echo "Ok, running \"bundle exec pod trunk push TealiumPrismFirebase.podspec\" now."; bundle exec pod trunk push TealiumPrismFirebase.podspec; break;;
